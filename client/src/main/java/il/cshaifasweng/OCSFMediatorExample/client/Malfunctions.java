...
public class Malfunctions {
    ...
    void initialize()
    {
        ...
        updateSpaceCombobox.getItems().add("Faulty");
    }
    ...
    void UpdateSpace(ActionEvent event) throws IOException {
        ...
        if (parkingSpace.getStatus() == 1){
            errorLoginMassage.setText("Parking space is already occupied, need to remove the car first before changing status");
            return;
        }
        ...
        if (update.equals("Faulty") & (parkingSpace.getStatus() == -1)){
            errorLoginMassage.setText("Parking space already is broken");
            return;
        }
        if (update.equals("Reserved") & (parkingSpace.getStatus() == 2)){
            errorLoginMassage.setText("Parking space already is reserved");
            return;
        }
